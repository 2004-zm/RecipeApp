...
public class RecipeDatabase //only this class will be able to access the database
{
    ...
    public static async Task Init() //init() initiates
    {
        if (_database != null)
            return;

        var dbPath = Path.Combine(FileSystem.AppDataDirectory, "recipes.db"); //this only finds where the path is of the database 
        _database = new SQLiteAsyncConnection(dbPath);
        await _database.CreateTableAsync<Recipe>(); //this stops and creates a table of Recipe
    }
    ...
    public static Task<List<Recipe>> GetRecipesAsync()
        => _database.Table<Recipe>().ToListAsync(); //this is like toString but converts it into a list 
...
//async makes not the ui not to stop will db is making operations ze
